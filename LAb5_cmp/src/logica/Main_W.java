...
public class Main_W extends JFrame {  
	...
	private void pintarCalle(Corredor corredor, Component[] botones)
	{
		...
		if (corredor.getPosicion()>-1 && corredor.getCalleAsignada().getCasilla(corredor.getPosicion()).tieneArbol()) {
			JButton boton = (JButton) botones[corredor.getPosicion()];
	...
	public void pintarArboles(Component[] botones)
	{
		for( int i = 0; i < (botones.length - 1) ; i++)
		{
			...
			} else if (carrera.getLiebre().getPosicion()>-1 && carrera.getLiebre().getCalleAsignada().getCasilla(carrera.getLiebre().getPosicion()).tieneArbol()) {
				JButton boton = (JButton) botones[carrera.getLiebre().getPosicion()];
				boton.setIcon(new ImageIcon(getClass().getResource("/img/liebre_durmiendo.JPG")));
				boton.setDisabledIcon(new ImageIcon(getClass().getResource("/img/liebre_durmiendo.JPG")));
	...
	private void jugar(int i)
	{
		if ( carrera.isJugadaCorrecta(i))
		{
			...
			if(carrera.getCorredorActivo().isDurmiendo())
				JOptionPane.showMessageDialog(this, "Has caido en un árbol. Se siente pierdes un turno.");
